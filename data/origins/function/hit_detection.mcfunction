...
execute if score $source ehm._ = $source ehm._ as @e[predicate=hit_match:is_source,limit=1,predicate=origins:fist,team=Bumblebee] if score $victim ehm._ = $victim ehm._ as @e[predicate=hit_match:is_victim,limit=1] run function origins:bumblebee/poison
execute if score $source ehm._ = $source ehm._ as @e[predicate=hit_match:is_source,limit=1,team=Arachnid] if score $victim ehm._ = $victim ehm._ as @e[predicate=hit_match:is_victim,limit=1] run function origins:arachnid/web
